...
@Component({
  selector: 'app-user-profile-advisor',
  templateUrl: './user-profile-advisor.component.html',
  styleUrls: ['./user-profile-advisor.component.css']
})
export class UserProfileAdvisorComponent implements OnInit {
  ...
  getUserInfo(){
    this._userApiService.getUserInfo().subscribe(
      (data:any) => {
        ...
        // if(data.data.data.aboutMe == undefined || data.data.data.aboutMe == "")
      ...
      this._userApiService.updateProfile(url, bodyImg).subscribe(
        (data:any) => {
          this._userApiService.updateProfile(url, body).subscribe(
            (data:any) => {
              this.router.navigate(['user-profile-advisor']);
            },        
          );  
        },
        (err) => Swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'Error in updating the profile image!'
        })         
      );
    }
    else{
